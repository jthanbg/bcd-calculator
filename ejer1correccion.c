...
#define SIGNO_SUMA " + "
#define SIGNO_MULTIPLICACION " * "
#define SIGNO_IGUAL " = "
#define INGRESO_MAX_BUFFER 100
//IMPRESIÓN.
...
size_t multiplicar_por_un_digito(const char a[], size_t na, char b, char r[]){
    ...
    if (acarreo > 0) {
        r[i++] = acarreo; //No surgió ningún error relacionado al acarreo, entonces lo dejé sin modificaciones.
    }
...
int leer_numero(unsigned long* num) {
    char buffer[INGRESO_MAX_BUFFER];

    fgets(buffer, INGRESO_MAX_BUFFER, stdin); //Se asume que el usuario siempre ingresará dos números correctamente.
...
void mostrar_operaciones_bcd(unsigned long x, unsigned long y) {
...
int main(){
    ...
    while (1) {
        ...
        if (!leer_numero(&x) || !leer_numero(&y)) {
            break; //Esto hace que se repita el while mientras se ingresan números.
        }

        mostrar_operaciones_bcd(x, y); //La impresión de la suma y multiplicación quedó en una función auxiliar, dejando más prolijo al int main().
    }
