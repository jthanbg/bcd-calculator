...
void imprimir_bcd(const char bcd[], size_t digitos){
    ...
    char inver[digitos];
    for (size_t i = 0; i < digitos; i++) { //MG: esto es innecesario (podrías aprovechar que sabes la cantidad de dígitos para iterar al revés al imprimir)
    ...
    for(int i = 0; i < digitos; i++){
        ...
        if (digitos_res > 0 && digitos_res % 3 == 0) { 
            printf(DELIMITADOR_MILES); //MG: tema etiquetas/macros
        }
...
int exponencial(int x, int n){ //MG: esto es innecesario, podrías ir guardándote el número correspondiente a la potencia actual y aumentarlo con cada iteración
    int i = 0;
    int resultado = 1;
    while (i < n){
        resultado *= x;
...
size_t binario_a_bcd(unsigned long binario, char bcd[]) {
    ...
    if (binario == 0) {
        bcd[0] = 0; //MG: acá pondría bcd[digitos]
        return 1;
    }
    /
    ...
    while (binario > 0) {
        ...
        binario /= 10; //Esto borra el último dígito del número. //MG: hablemos sobre modificar variables externas
        digitos++;
    }
...
size_t sumar_bcd(const char a[], size_t na, const char b[], size_t nb, char r[]){
    size_t i = 0;
    size_t num_len = (na > nb) ? na : nb; //MG: muy lindo el ternario 
    int acarreo = 0;
    ...
    if (acarreo > 0) {
        r[i] = acarreo;
        return i + 1; //MG: si quisieras podrías poner r[i++]=acarreo; y ahorrarte este return, después devolves i que ya está una más adelante. 
    }
...
size_t multiplicar_por_un_digito(const char a[], size_t na, char b, char r[]){
    ...
    if (acarreo > 0) { //MG: ojo con el caso de tener un acarreo >9...
        r[i] = acarreo;
        return i + 1;
    }
...
int leer_numero(unsigned long* num) { //MG: hablemos sobre fgets 
    char buffer[100]; //MG: hablemos sobre macros/etiquetas
    int i = 0;
    int c = getchar();
...
int main(){
    ...
    while(1){ 
        ...
        int b = y % 10;
        //MG: el tema de imprimir la suma y la multiplicación hubiese estado bueno que fuese hecho en una función auxiliar
        imprimir_bcd(bcdx, len_x);
        printf(" + "); //MG: hablemos de macros/etiquetas
